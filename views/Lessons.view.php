...
<script>
    ...
    {   
        ...
        // Gathers the users date input
        let text2 = document.querySelector(".js-search2").value;
        // Gathers the users location selection
        const formData = new FormData();
        formData.append("text",text);
        formData.append("text2",text2);
        // Creates a form to be taken into the API
        // Then the user date and location is added 
        // To the formData
        fetch("Placeholder.php",{
            "method": "POST",
            "body":formData
            // Connection is made to the API taking in formData
        }).then(function(response){
            return response.json();
            // Gets result from fetch
        }).then(function(data){
            handle_result(data);
            // Sends result to handle_result function
        });
    }
    function handle_result(result){
        let result_div=document.querySelector(".js-results")
        // Gathers the dropdown to append values to
        while(result_div.firstChild){
            result_div.removeChild(result_div.firstChild)
        };
        // Clears any previous dropdown options

        let obj = result;
        // changes result to obj
        for(let i = obj.length - 1; i >= 0; i--){
            console.log(obj[i].Time)
            let option = document.createElement("option")
            option.textContent = obj[i].Time;
            option.value = obj[i].Time;
            // Loops through each obj in the returned data
            // And adds it through create element to
            // turn it into a option and then .textcontent
            // and .value to assign a value

            result_div.appendChild(option);
            // then it appends this value to the result
            // so it can be put into the dropdown
        }
