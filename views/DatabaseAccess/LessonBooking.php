...
session_start();
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(date("N", strtotime(str_replace("/","-",$date))) >= 6){
            $ErrorArray[] = "Day selected can not be a weekend!";
            $Error_Count +=1;
        }
        // checks to ensure the date isnt a weekend
        ...
        if ($Error_Count == 0){
            ...
            VALUES ('$id','Lesson','$location','$date','$time')";
            $conn->query($sql);
        ...
        header(header: "Location: ../Lessons.view.php");
